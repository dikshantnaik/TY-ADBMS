...
--
-- Host: localhost
-- Generation Time: Aug 09, 2022 at 06:45 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetStudent` (IN `susername` VARCHAR(20))   SELECT * FROM `students` WHERE username = susername$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertReviews` (IN `course_id` INT, IN `susername` VARCHAR(50), IN `sreview` VARCHAR(100))   INSERT INTO review VALUES(NULL,course_id,(SELECT studentid FROM students WHERE username = susername),NULL,sreview)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Register` (IN `susername` VARCHAR(50), IN `sname` VARCHAR(50), IN `scourse` VARCHAR(50), IN `spassword` VARCHAR(500))   INSERT INTO students VALUES(NULL,susername,sname,scourse,spassword)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `removeItemFromCart` (IN `susername` VARCHAR(50), IN `c_id` INT)   DELETE FROM cart WHERE sid = (SELECT studentid from students WHERE username=susername) and  cid= c_id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Test` (IN `test23` INT)   INSERT INTO test VALUES(test23)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `testqw` ()   BEGIN
  DECLARE _sid int;
  SELECT  students.studentid INTO _sid FROM students WHERE students.studentid= 1;
  SELECT _sid as siid;
END$$
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `login` (`susername` VARCHAR(50), `spassword` VARCHAR(50)) RETURNS INT(1)  RETURN(EXISTS(SELECT * FROM students WHERE students.username = susername AND students.username = spassword))$$

CREATE DEFINER=`root`@`localhost` FUNCTION `testF` (`no` INT) RETURNS INT(11)  return no$$
...
(14, 1, 17, 0, 'I love it ..'),
(15, 1, 1, NULL, 'NICE'),
(16, 1, 1, NULL, 'Nice Course'),
(17, 1, 1, NULL, 'Nice adwd');
...
(18, 'aUser', 'awdadw', 'awd', 'esfsf'),
(19, 'hahahh', 'ahadhwdin', 'owanidn', 'adfadfawd'),
(20, 'Prasad', 'adwdw', 'wadawd', 'awdawd'),
(21, 'dikshant2323', 'Dikshant', 'bsc', '5E884898DA28047151D0E56F8DC6292773603D0D6AABBDD62A11EF721D1542D8'),
(22, 'awdadw', 'awdawd', 'wad', 'wdadw'),
(25, 'FuckYou', 'Dikshant', 'bsc', 'password'),
(28, 'FuckYoasu', 'Dikshant', 'bsc', 'password');
...
--
ALTER TABLE `students`
  MODIFY `studentid` int(50) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=29;
